  ...
  activate() {
    ...
    let cmdRefresh = atom.commands.add('atom-workspace', {
        'dbex:refresh': (element) => {
          ...
          // console.log('element-context-click', element);
          ...
          let connInfo = this.treeView._getDatasetFromElement(target);

          console.log(
            target,
            this.treeView._getDatasetFromElement(target)
          );
    ...
    // temp1.closest('li.has-children[data-engine]')
      ...
      let renderResults = (totalRecordsToRender)=>{
        if (Array.isArray(response) && response[0].constructor.name === "TreeItem") {
          ...
          let setResult = (responseData, resultView)=>{
            ...
            //     // layout: 'fluid',
            //   });
              // TODO: add this responseData to a created array to keep individual tabs destroyables/disposables
              // this.disposables.add(datatable); //TODO: must destroy when tab is closed instead on general dispose
            // }
...
// console.log('paneName', paneName);
  ...
  _createResultArea(resultName, nodeData, color, engine, connectionName, engineName) {
    ...
    // this.disposables.add(resultView); //TODO: must destroy when tab is closed instead on general dispose
...
//<i class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i>
    ...
    resultView.onShowLog = (uuid) => {
      // console.log("show the log", uuid, engine);
      engine.getLogger().scope = nodeData.name;
    ...
    resultView.onDestroy = (uuid) => {
      // console.log("destroying result ", nodeData.name, connectionName, uuid);
      // alert(nodeData.name);

      //TODO: destroy resultView objects
  ...
  _updateHistory(name, connection, content) {
    // console.log("query has to be saved ", name, connection, content, this.queryHistory);
  ...
  _loadHistory(connectionName, engine) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      // console.log('loading history data', JSON.parse(data));
      // this.queryHistory[connectionName] = parsedData.length > 0 ? parsedData : {};
...
// console.log('history keys', historyKeys);
    ...
    // console.log('_loadHistory', this.queryHistory);
  },
  ...
  _showEditConnection() {
    ...
    if (nodeConn && nodeConn.connection) {
      ...
      // console.log("_showEditConnection .... -->", nodeConn.node);
  ...
  _saveFile(contentObj, filePath) {
    ...
    // console.log("saving conent to file ====>", contentObj, JSON.stringify(contentObj));
