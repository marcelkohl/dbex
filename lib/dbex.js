...
export default {
  ...
  singleClickAction(nodeInfo) {
    ...
    if (nodeInfo && nodeInfo.action) {
      ...
      engine.resolveActionClick(
        nodeInfo.action,
        nodeInfo,
        (e)=>console.log("return success from single click ===> ", e),
  ...
  _clickOnStatusBar(selectedConnection) {
    ...
    if (engine) {
      ...
      // let resultView = this._createResultArea(
      //   resultName,
      //   { // this data goes through result options when clicked
      //     name: selectedConnection.name
      //   },  // was nodeData
      //   selectedConnection.color ? `#${selectedConnection.color}` : "transparent",
      //   engine,
      //   selectedConnection.name,
      //   selectedConnection.engine
      // );
      //
      // resultView.setQuery('SOME QUERY HERE');
      // resultView.setDataVisibility(false);
      //
      // atom.workspace.open(
      //   resultView, {
