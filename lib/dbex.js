...
import NewConnectionDialog from './view/new-connection-dialog';
...
export default {
  ...
  activate() {
    ...
    let nconn = new NewConnectionDialog();
    nconn.onSave = (data)=>alert(data);
    // console.log(nconn);

    const connect = document.createElement('div');
    connect.innerHTML = this.getConnectView();
    connect.getElementsByClassName('add-connection')[0].addEventListener('click', () => nconn.show())
    // atom.tooltips.add(button, {title: 'Here'});
  ...
  getConnectView() {
    ...
    <div tabindex="-1" class="dbex-controls">
      <header class="header">
        <!-- span class="header-item close-button pull-right" data-original-title="" title="">
          <i class="icon icon-x clickable"></i>
        </span>
        <span class="header-item description">Find in Current Buffer</span -->
        <span class="header-item options-label pull-right">
          <span class="btn-group btn-toggle btn-group-options">
            ...
            <!-- button class="btn option-selection" data-original-title="" title=""><svg class="icon"><use href="#find-and-replace-icon-selection"></use></svg></button><button class="btn option-whole-word" data-original-title="" title=""><svg class="icon"><use href="#find-and-replace-icon-word"></use></svg>
            </button -->
          </span>
        </span>
      </header>
