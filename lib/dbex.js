        ...
        if (response) {
          if (Array.isArray(response) && response[0].constructor.name === "TreeItem") {
            ...
            if (resultArea.length === 0) {
              ...
              resultView.toolbar.message = response.data.length + " rows in ~" + ((Date.now() - timestamp)/100)+"s";
              ...
              resultArea[0].querySelector('.toolbar .options-label').innerHTML = response.data.length + " rows in ~" + ((Date.now() - timestamp)/100)+"s";
