    ...
    if (response) {
      let renderResults = (totalRecordsToRender, timeFinished)=>{
        ...
        if (Array.isArray(response) && response[0].constructor.name === "TreeItem") {
          ...
          let setResult = (responseData, resultView)=>{resultView.resultData = responseData;};
          ...
          console.log(timeFinished, timestamp);
          if (resultView && response.columns && response.columns.length > 0) {
