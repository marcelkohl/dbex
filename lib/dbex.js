...
export default {
  ...
  doubleClickAction(nodeData) {
    console.log("Dblclick", nodeData);

    if (nodeData && nodeData.engine) {
      console.log(nodeData.name);
      this.treeView.updateControls(nodeData.name, '<i class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i>');

      let engine = this.engines[nodeData.engine];
      let onStructureFail = (message)=>{
      ...
      let onStrutureSuccess = (structure)=>{
        this.treeView.updateNode({
          name: nodeData.name,
          label: nodeData.label,
          icon: engine.getIconClass() || 'icon-database',
          details: "",
          children:structure,
          classes: "structured",
          datasets: nodeData
      ...
      engine.getStructure(nodeData, onStrutureSuccess, onStructureFail);
    }
  },
