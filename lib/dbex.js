...
export default {
    ...
    this._engines[connInfo.engine].refreshNode(
      ...
      (response)=>{
        if (response.constructor.name === "TreeItem") {
          this._treeView.updateNode(response);
        }
      }
    );
  },
  ...
  _engineCall(nodeInfo, callback) {
    ...
    if (nodeInfo.node && engine) {
      ...
      let onFail = (message)=>this.showError(message, "dbex engine failed");
    ...
    if (response) {
      let renderResults = (totalRecordsToRender)=>{
        if (Array.isArray(response) && response[0].constructor.name === "TreeItem") {
          this._treeView.updateNodeChild(nodeData.name, response);
          ...
          if (resultArea.length === 0) {
            ...
            resultView.toolbar.message = (response.recordsAffected || (response.data ? response.data.length : 0)) + " rows in ~" + ((Date.now() - timestamp)/1000)+"s";
            ...
            if (response.columns && response.columns.length > 0) {
              let responseToRender;
              let resultView = this._resultViews[document.getElementsByClassName(resultName)[0].dataset.uuid];
      ...
      if (totalRecords > this._packageConfig.MaximumLimitRecordsResult) { //this._limitRecordsResult) {
        this.confirmLimitRecords(totalRecords, this._packageConfig.MaximumLimitRecordsResult, renderResults);
      } else {
  ...
  _updateHistory(name, connection, content) {
    ...
    if (content === undefined) {
      ...
      this._queryHistory[connection][name] = content;
    }
    ...
    if (fs.existsSync(filePath)) {
      ...
      historyKeys.forEach((historyKey, i) => {
        ...
        let content = new ResultSet({
          query: this._queryHistory[connectionName][historyKey]
        });
  ...
  _removeConnection(connNameToRemove) {
    ...
    this._treeView.removeNodeByName(connNameToRemove);
  },
  ...
  _clickOnStatusBar(selectedConnection) {
    let engine = selectedConnection.engine ? this._engines[selectedConnection.engine] : undefined;
