...
export default {
  ...
  _createStatusBarNotification() {
    this._statusBarNotification = new StatusBarNotification("");
  ...
  _onSuccess(response, timestamp, connectionName, engineName, engine, nodeData) {
    if (response) {
      ...
      if (totalRecords > this._limitRecordsResult) {
        this.confirmLimitRecords(totalRecords, this._limitRecordsResult, renderResults);
      } else {
