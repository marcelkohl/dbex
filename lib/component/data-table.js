...
export default class DataTable {
  constructor(targetQuery, {columns, data, id}) {
    ...
    tBody.onclick = (el)=>this.dotheneedful(el.target);
    ...
    this._makeResizable(document.querySelector(`#${this._element.id} > table`));
      var clusterize = new Clusterize({
        ...
        callbacks: {
          ...
          clusterChanged: (e)=>{
            let row = table.querySelector(`[data-row="${this._lastRowIndex}"] > td`)
...
// console.log(window.getComputedStyle(col));
  ...
  dotheneedful(sibling) {
    if (sibling != null) {
      ...
      if (sibling.parentElement.dataset.row === "0") {
