...
export default class CreateConnection {
            ...
            <div class="controls">
              ...
              <div class="controls">
                <div class="editor-container">
                  <atom-text-editor class="editor mini" mini="" data-encoding="utf8" data-grammar="text plain null-grammar" tabindex="-1" id="connection-name" type="number" data-original-title="" title=""></atom-text-editor>
                </div>
              </div>
            </div>
            <div class="controls">
              ...
              <select id="connection" class="form-control connection-manager" data-original-title="" title="">
  ...
  /**
   * @param  DbManager[] managers
   */
  show(managers) {
    if (!this.dialogPanel) {
      let options = '';

      Object.entries(managers).forEach((manager) => {
        let [name, obj] = manager;
        let info = obj.getConnectionSettings();
        // console.log(name, info);
        options += `<option value="${name}">${info.label}</option>`;
      });

      this.element.getElementsByClassName("connection-manager")[0].innerHTML = '<option value="">Select...</option>' + options;
