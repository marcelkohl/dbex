...
export default class TreeView {
  ...
  _parseClick(event) {
    ...
    if (target.classList.contains('item-action') || (target.tagName === "I" && target.parentElement.classList.contains('item-action'))) {
  ...
  set onDoubleClick(onDoubleClick) {
    this._onDoubleClick = (event)=>{
      let nodeInfo = this._parseClick(event);
      onDoubleClick(nodeInfo ? nodeInfo.node : undefined);
    }
  }
