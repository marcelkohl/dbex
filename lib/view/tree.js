...
class TreeNode {
  constructor(item) {
    const {label, name, icon, children, details, collapsed, datasets, classes} = item;
    ...
    const controlsElement = document.createElement('span');
    controlsElement.classList.add('item-controls');

    this.element = document.createElement('li');

    if (children) {
      this.element.classList.add('list-nested-item', 'list-selectable-item', collapsed, classes);
      ...
      root.appendChild(controlsElement);
...
export default class TreeView {
  ...
  updateControls(nodeName, content) {
    ...
    this.list.querySelector(`[data-name="${nodeName}"] .item-controls`).innerHTML = content;
  }
