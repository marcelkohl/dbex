...
class TreeNode {
  constructor(item) {
    ...
    // if (children) {
      ...
      let colorNode = datasets ? (datasets.color ? `#${datasets.color}` : "transparent") : "transparent";
      root.style.borderLeft = `3px solid ${colorNode}`;
      ...
      //           label: action.name,
      //           command: 'dbex:action',
      //           created: (e)=>{console.log('created' ,e); this.label = 'xxxxxx-1';}
      //         });
      //       });
      //
      //       let menuEl = {};
      //       menuEl[`[data-name='${name}'] > div`] = menuOptions;
      //
      //       atom.contextMenu.add(menuEl);
      //       // console.log(menuEl);
      //
      //       // let actionElKey = `[data-name='${name}'] > div`;
      //       // console.log('action--->', name, this.element.querySelector(`[data-name='${name}'] > div`))
      //   // }, 3000);
      // }
...
export default class TreeView {
  ...
  getSelected() {
    ...
    return nodeSelected ? this._getDatasetFromElement(nodeSelected) : {};
  }

  removeElementByConnectionName(name) {
    let element = this.element.querySelector(`[data-name="${name}"]`);

    // console.log(name, element, this.element);
    if (element) {
  ...
  getConnectionData(connectionName) {
    // console.log('getConnectionData connectionName', connectionName)
    // console.log('getConnectionData element', document.querySelector(`[data-engine][data-name='${connectionName}']`));
    return document.querySelector(`[data-engine][data-name='${connectionName}']`).dataset;
  }

  _getDatasetFromElement(target) {
    let currentNode;
    let collapsed = false;
    ...
    return {
      ...
      node: Object.assign({}, currentNode.dataset)
    };
  }
  ...
  _parseClick(event) {
    ...
    return this._getDatasetFromElement(target);
  }
