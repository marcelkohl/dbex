...
export default class DbexResult {
  constructor({name, title, icon, color, uuid, allowedLocations, customHeaderOptions, grammar}) {
    this.destroyables = [];
  ...
  set queryGrammar(grammar) {
    if (grammar) {
      this.query.setGrammar(atom.grammars.selectGrammar(grammar));
    }
  }
  ...
  _observeResizableAreas(resultArea, dataElement, otherElements) {
    this._resizeObserver = new ResizeObserver(entries => {
      ...
      remainingSpace = remainingSpace < 145 ? 145 : remainingSpace;
